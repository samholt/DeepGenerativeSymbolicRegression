...
from .gen_utils import remap_independent_vars_to_monotic, remap_to_include_all_vars
...
class Generator(object):
    ...
    def __init__(self, params):
        self.equal_prob_independent_vars = params.equal_prob_independent_vars
        self.remap_independent_vars_to_monotic = params.remap_independent_vars_to_monotic
        self.force_all_independent_present = params.force_all_independent_present
        self.max_ops = params.max_ops
        self.max_len = params.max_len
        self.max_independent_vars = params.max_independent_vars
        self.lower_nbs_ops = params.lower_nbs_ops
        self.create_eqs_with_constants = params.create_eqs_with_constants
    ...
    def get_leaf(self, curr_leaves, rng):
        if self.equal_prob_independent_vars:
            return [list(self.variables.keys())[rng.randint(low=0, high=len(self.variables.keys()))]]
        else:
            if curr_leaves:
    ...
    def infix_to_sympy(cls, infix, variables, rewrite_functions, no_rewrite=False):
        ...
        try:
            ...
            raise ImAccomulationBounds("ImAccomulationBounds")
        if expr.has(sp.I) or expr.has(AccumBounds):
            raise ValueErrorExpression("ValueErrorExpression")
        if not no_rewrite:
    ...
    def process_equation(self, infix):
        ...
        if not symbols:
            raise NotCorrectIndependentVariables(
                f"NotCorrectIndependentVariables - No variables in the expression, skip")
        ...
        for s in symbols:
            if not len(set(self.var_symbols[:self.pos_dict[s]]) & symbols) == len(self.var_symbols[:self.pos_dict[s]]):
                raise NotCorrectIndependentVariables(
                    f"NotCorrectIndependentVariables - Variable {s} in the expressions, but not the one before")
                # return None, f"Variable {s} in the expressions, but not the one before"

        if self.create_eqs_with_constants:
            f = remove_root_constant_terms(
                f, list(self.variables.values()), 'add')
            f = remove_root_constant_terms(
                f, list(self.variables.values()), 'mul')
    ...
    def generate_equation(self, rng):
        ...
        """
        # t0 = time()
        nb_ops = rng.randint(self.lower_nbs_ops, self.max_ops + 1)
        f_expr = self._generate_expr(nb_ops, rng, max_int=1)
        # print(f't0 {time()-t0}')
        if self.remap_independent_vars_to_monotic and any([True for i in f_expr if 'x' == i[0]]):
            # f_expr = remap_independent_vars_to_monotic(f_expr)
            f_expr = remap_to_include_all_vars(f_expr)
            # f_expr = [f for f in f_expr if f != 'sin']
            if self.force_all_independent_present:
                if not self.var_symbols[-1] in f_expr:
                    raise ValueErrorExpression(
                        "Does not contain all independent leaf variables")
                if len(eq_sympy_prefix_to_token_library(f_expr)) > self.max_len:
                    raise ValueErrorExpression("Seqeunce Length too long")
        # print(f't1 {time()-t0}')
        if max([int(i.split('_')[1]) for i in f_expr if i[0] == 'x']) > self.max_independent_vars:
            raise ValueErrorExpression("Expression contains too many vars")
        # print(f't2 {time()-t0}')
