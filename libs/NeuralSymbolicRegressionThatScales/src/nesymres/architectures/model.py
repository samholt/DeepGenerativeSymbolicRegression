...
class Model(pl.LightningModule):
    def __init__(
        ...
        self.decoder_transfomer = nn.TransformerDecoder(
            decoder_layer, num_layers=cfg.dec_layers)
    ...
    def create_sinusoidal_embeddings(self, n_pos, dim, out):
        ...
        out[:, 0::2] = torch.FloatTensor(
            np.sin(position_enc[:, 0::2]))  # Odd indices
        out[:, 1::2] = torch.FloatTensor(
            np.cos(position_enc[:, 1::2]))  # Even indices
        out.detach_()
    ...
    def compute_loss(self, output, trg):
        output = output.permute(
            1, 0, 2).contiguous().view(-1, output.shape[-1])
    ...
    def fitfunc(self, X, y, cfg_params=None, return_prefix_skeleton=True):
        ...
        with torch.no_grad():
            ...
            enc_src = src_enc.unsqueeze(1).expand(
                (1, cfg_params.beam_size) + src_enc.shape[1:]).contiguous().view(shape_enc_src)
            print(
            ...
            generated_hyps = BeamHypotheses(
                cfg_params.beam_size, self.cfg.length_eq, 1.0, 1)
            done = False
            # Beam Scores
            beam_scores = torch.zeros(
                cfg_params.beam_size, device=self.device, dtype=torch.long)
            beam_scores[1:] = -1e9
            ...
            while cur_len < self.cfg.length_eq:
                ...
                assert output[:, -1:,
                              :].shape == (cfg_params.beam_size, 1, self.cfg.length_eq,)
                ...
                assert len(next_scores) == len(
                    next_words) == 2 * cfg_params.beam_size
...
if __name__ == "__main__":
    model = SetTransformer(n_l_enc=2, src_pad_idx=0, trg_pad_idx=0, dim_input=6, output_dim=20, dim_hidden=40, dec_layers=1, num_heads=8, dec_pf_dim=40, dec_dropout=0, length_eq=30, lr=0.001,
                           num_inds=20, ln=True, num_features=10, is_sin_emb=False, bit32=True, norm=False, activation='linear', linear=False, mean=torch.Tensor([1.]), std=torch.Tensor([1.]), input_normalization=False)
