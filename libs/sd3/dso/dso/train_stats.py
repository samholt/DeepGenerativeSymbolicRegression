...
class StatsLogger():
    ...
    def setup_output_files(self):
        ...
        if self.output_file is not None:
            ...
            self.positional_entropy_output_file = "{}_positional_entropy.npy".format(
                prefix)
            self.top_samples_per_batch_output_file = "{}_top_samples_per_batch.csv".format(
                prefix)
            self.cache_output_file = "{}_cache.csv".format(prefix)
            self.token_counter_output_file = "{}_token_count.csv".format(
                prefix)
    ...
    def save_stats(self, r_full, l_full, actions_full, s_full, invalid_full, r, l,
        ...
        if self.output_file is not None:
            ...
            a_ent_full = np.mean(np.apply_along_axis(
                empirical_entropy, 0, actions_full))
            ...
            r_avg_sub = np.mean(r)
            l_avg_sub = np.mean(l)
            a_ent_sub = np.mean(np.apply_along_axis(
        ...
        if self.save_all_epoch:
            ...
            df.to_csv(self.buffer_all_programs, mode='a',
    ...
    def save_results(self, positional_entropy, top_samples_per_batch, r_history, pool, n_epochs, n_samples):
        ...
        if self.hof is not None and self.hof > 0:
            ...
            if Program.task.stochastic:
                ...
                programs = [from_token_string(str_tokens)
                            for str_tokens in keys]
            ...
            # For deterministic Programs, just use the cache
                ...
                programs = list(Program.cache.values())

            r = [p.r for p in programs]
            # Indices of top hof Programs
            i_hof = np.argsort(r)[-self.hof:][::-1]
            ...
            columns = ["r", "count_on_policy", "count_off_policy",
                       "expression", "traversal"] + eval_keys
            hof_results = [result[:-1] + [result[-1][k]
                                          for k in eval_keys] for result in results]
            ...
            if self.save_pareto_front:
                ...
                pareto_efficient_mask = is_pareto_efficient(
                    costs)  # List of bool
                ...
                eval_keys = list(results[0][-1].keys())
                columns = ["complexity", "r", "count_on_policy",
                           "count_off_policy", "expression", "traversal"] + eval_keys
                pf_results = [result[:-1] + [result[-1][k]
                                             for k in eval_keys] for result in results]
