...
Batch = namedtuple(
    "Batch", ["actions", "obs", "priors", "lengths", "rewards", "on_policy", "data_to_encode", "tgt"])
...
def get_samples(batch, key):
    ...
    if batch.data_to_encode.size != 0:
        data_to_encode = batch.data_to_encode[key]
    else:
...
class ProgramQueueMixin():
    ...
    def _make_batch(self, samples):
        ...
        data_to_encode = np.stack([s.data_to_encode for s in samples], axis=0)
        tgt = np.stack([s.tgt for s in samples], axis=1)
        batch = Batch(actions=actions, obs=obs, priors=priors,
                      lengths=lengths, rewards=rewards, on_policy=on_policy, data_to_encode=data_to_encode, tgt=tgt)
        return batch
