...
class GP():
    ...
    def __init__(self, dataset, metric="nmse", population_size=1000,
                 generations=1000, n_samples=None, tournament_size=5,
                 p_crossover=0.5, p_mutate=0.5,
                 const_range=[-1, 1], const_optimizer="scipy",
                 const_params=None, seed=0, early_stopping=True,
                 threshold=1e-12, verbose=True, protected=False,
                 pareto_front=False,
                 # Constraint hyperparameters
                 constrain_const=False,
                 constrain_trig=False,
                 constrain_inv=False,
        ...
        self.break_out_after_same_fitness_seen_limit = 2
        self.break_out_after_same_fitness_seen = 0
        self.previous_fitness = 9999
        self.top_hf_person = None
    ...
    def acc_iid(self, ind):
        f = self.toolbox.compile(expr=ind)
        y_hat_test = f(*self.dataset.X_test.T)
        y_test = self.dataset.y_test
        return acc_tau(y_test, y_hat_test)
    
    def acc_ood(self, ind):
        f = self.toolbox.compile(expr=ind)
        y_hat_test = f(*self.dataset.X_test_ood.T)
        y_test = self.dataset.y_test_ood
        return acc_tau(y_test, y_hat_test)
    ...
    def train(self):
        ...
        print(f'b random_state: {random.getstate()}')
        print(f'b np.random_state: {np.random.get_state()}')
        ...
        # stats_acc_iid = tools.Statistics(lambda p : self.acc_iid(p))
        # stats_acc_iid.register("avg", np.mean)
        # stats_acc_iid.register("max", np.max)
        # stats_acc_ood = tools.Statistics(lambda p : self.acc_ood(p))
        # stats_acc_ood.register("avg", np.mean)
        # stats_acc_ood.register("max", np.max)

        stats_fit = tools.Statistics(lambda p : p.fitness.values[0])
        # stats_fit = tools.Statistics(lambda p : p.fitness.values[0] if p.fitness.values else 998)
        ...
        mstats = tools.MultiStatistics(fitness=stats_fit,
                                        size=stats_size)
                                        # acc_iid=stats_acc_iid,
                                        # acc_ood=stats_acc_ood)
        ...
        return ind_best, logbook, self.hof
