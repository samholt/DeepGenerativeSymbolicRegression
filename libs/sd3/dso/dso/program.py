...
from dso.utils import cached_property, log_and_print
...
def _finish_tokens(tokens):
    ...
    if -n_objects in (dangling - 1):
        ...
        tokens = np.append(tokens, np.random.choice(
            Program.library.input_tokens, size=dangling[-1]))
...
class Program(object):
    ...
    def __init__(self, tokens=None, on_policy=True, custom_task=None):
        ...
        if tokens is not None:
            self._init(tokens, on_policy, custom_task=custom_task)

    def _init(self, tokens, on_policy=True, custom_task=None):

        self.traversal = [Program.library[t] for t in tokens]
        self.const_pos = [i for i, t in enumerate(
            self.traversal) if isinstance(t, PlaceholderConstant)]
        ...
        if Program.n_objects > 1:
            if not Program.protected:
                ...
                for trav in self.traversals:
                    val, invalid, self.error_node, self.error_type = Program.execute_function(
                        trav, X)
                    result.append(val)
                    invalids.append(invalid)
                self.invalid = any(invalids)
            else:
                result = [Program.execute_function(
                    trav, X) for trav in self.traversals]
            return result
        else:
            if not Program.protected:
                result, self.invalid, self.error_node, self.error_type = Program.execute_function(
                    self.traversal, X)
            else:
    ...
    def set_execute(cls, protected):
        ...
        if protected:
            ...
            class InvalidLog():
                ...
                def __init__(self):
                    # One of ['divide', 'overflow', 'underflow', 'invalid']
                    self.error_type = None
                    self.error_node = None  # E.g. 'exp', 'log', 'true_divide'
                    # Flag for whether a warning has been encountered during a call to Program.execute()
    ...
    def evaluate(self):
        ...
        if "r" not in self.__dict__:
            log_and_print("WARNING: Evaluating Program before computing its reward."
    ...
    def print_stats(self):
        ...
        log_and_print("\tReward: {}".format(self.r))
        log_and_print("\tCount Off-policy: {}".format(self.off_policy_count))
        log_and_print("\tCount On-policy: {}".format(self.on_policy_count))
        log_and_print("\tOriginally on Policy: {}".format(
            self.originally_on_policy))
        log_and_print("\tInvalid: {}".format(self.invalid))
        log_and_print("\tTraversal: {}".format(self))

        if Program.n_objects == 1:
            log_and_print("\tExpression:")
            log_and_print("\n{}\n".format(indent(self.pretty()[0], '\t  ')))
        else:
            for i in range(Program.n_objects):
                log_and_print("\tExpression {}:".format(i))
                log_and_print("\n{}\n".format(
                    indent(self.pretty()[i], '\t  ')))
