...
def dataset_loader(train_dataset, test_dataset, batch_size=1024, valid_size=0.20):
    ...
    test_loader_h = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=0)
    return train_loader, valid_loader, test_loader_h, valid_idx, train_idx


def init_weights(m):
    for name, param in m.named_parameters():  # pylint: disable=unused-variable
...
def evaluate_fun(args):
    ...
    if type(fun) == list and not len(fun):
        return []
    f = types.FunctionType(fun, globals=globals(), name="f")  # pyright: ignore
    try:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            evaled = f(*support)  # pyright: ignore  # pylint: disable=not-callable
...
def return_dict_metadata_dummy_constant(metadata):
    dict = {key: 0 for key in metadata.total_coefficients}  # pylint: disable=redefined-builtin
...
def sample_symbolic_constants(eq: Equation, cfg=None) -> Tuple:
    ...
    if cfg:
        ...
        symbols_used = random.sample(set(eq.coeff_dict.keys()), used_consts)  # pyright: ignore
        for si in symbols_used:
            if si[:2] == "ca":
                consts[si] = round(float(Uniform(cfg.additive.min, cfg.additive.max).sample()), 3)  # pyright: ignore
            elif si[:2] == "cm":
                consts[si] = round(
                    float(Uniform(cfg.multiplicative.min, cfg.multiplicative.max).sample()), 3  # pyright: ignore
                )
            else:
                raise KeyError
    else:
