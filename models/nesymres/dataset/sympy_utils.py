...
# TODO: Double check this is not needed.
...
def remove_root_constant_terms_t(expr, variables, mode):
    ...
    if mode == "add" and expr.is_Add:
        args = [arg for arg in expr.args if any(str(x) in variables for x in arg.free_symbols) or (arg in [-1])]
        if len(args) == 1:
            expr = args[0]
        elif len(args) < len(expr.args):
            expr = expr.func(*args)
    elif mode == "mul" and expr.is_Mul:
        args = [arg for arg in expr.args if any(x in variables for x in arg.free_symbols) or (arg in [-1])]
        if len(args) == 1:
            expr = args[0]
        elif len(args) < len(expr.args):
            expr = expr.func(*args)
...
def extract_non_constant_subtree(expr, variables):
    ...
    while True:
        ...
        for i in range(n_arg):
            # if len(expr.args) == 1:
            #      #breakpoint()
            expr = expr.subs(expr.args[i], extract_non_constant_subtree(expr.args[i], variables))
...
def check_additive_constants(expr, variables):
    ...
    while True:
        ...
        for i in range(n_arg):
            # if len(expr.args) == 1:
            #      #breakpoint()
            if check_additive_constants(expr.args[i], variables) == True:  # noqa: E712
                return True
            else:
                continue
...
def add_multiplicative_constants(expr, multiplicative_placeholder, unary_operators=None):
    ...
    for sub_expr in expr.args:
        expr = expr.subs(
            sub_expr,
            add_multiplicative_constants(sub_expr, multiplicative_placeholder, unary_operators=unary_operators),
        )
...
def add_additive_constants(expr, placeholders, unary_operators=None):
    if unary_operators is None:
        unary_operators = []
...
def simplify_const_with_coeff(expr, coeff):
    ...
    if not (parent.is_Add or parent.is_Mul):  # pyright: ignore
        return expr
    # pylint: disable-next=undefined-loop-variable
    removed = [arg for arg in parent.args if len(arg.free_symbols) == 0]  # pyright: ignore
    if len(removed) > 0:
        # pylint: disable-next=undefined-loop-variable
        removed = parent.func(*removed)  # pyright: ignore
        # pylint: disable-next=undefined-loop-variable
        new_coeff = (coeff - removed) if parent.is_Add else (coeff / removed)  # pyright: ignore
