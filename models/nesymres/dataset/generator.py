...
class Generator(object):
    ...
    def __init__(self, params):
        ...
        # We do not no a priori how many coefficients an expression has, so to be on the same side we equal to
        # two times the maximum number of expressions
        ...
        self.words = (
            ...
            + [x for x in self.operators if x not in ("pow2", "pow3", "pow4", "pow5", "sub", "inv")]
            + digits
        )
    ...
    def return_local_dict(cls, variables=None):
        local_dict = {}
        for k, v in list(variables.items()):  # pyright: ignore
    ...
    def return_rewrite_functions(cls, params):
        ...
        assert all(x in ["expand", "factor", "expand_log", "logcombine", "powsimp", "simplify"] for x in r)
        return r
    ...
    def return_constants(self, expr_list):
        ...
        curr = Counter()
        curr["cm"] = [x for x in expr_list if x[:3] == "cm_"]  # pyright: ignore
        curr["ca"] = [x for x in expr_list if x[:3] == "ca_"]  # pyright: ignore
        return curr
    ...
    @classmethod
    def rewrite_sympy_expr(cfg, expr, rewrite_functions=None):  # pyright: ignore
        """
        Rewrite a SymPy expression.
        """
        expr_rw = expr
        for f in rewrite_functions:  # pyright: ignore
            if f == "expand":
                ...
                expr_rw = sp.logcombine(expr_rw, force=True)  # pyright: ignore
            elif f == "powsimp":
                expr_rw = sp.powsimp(expr_rw, force=True)  # pyright: ignore
            elif f == "simplify":
                # TODO: Check this does not occur.
    ...
    def infix_to_sympy(cls, infix, variables, rewrite_functions, no_rewrite=False):
        ...
        try:
            ...
            raise ImAccomulationBounds  # pylint: disable=raise-missing-from
    ...
    def sympy_to_prefix(cls, expr):
        ...
        if isinstance(expr, sp.Symbol):
            ...
            return ["div"] + [str(expr.p)] + [str(expr.q)]  # self.write_int(int(expr.p)) + self.write_int(int(expr.q))
        elif expr == sp.E:
            return ["E"]
        elif expr == sp.pi:
            return ["pi"]
