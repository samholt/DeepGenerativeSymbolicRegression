...
# def evaluate_model(
#     model_predict, benchmark_path, equation_idx, num_test_points, pointwise_acc_rtol, pointwise_acc_atol
# ):
#     """
...
#         X, y = get_data(gt_equation, num_variables, supp, num_test_points, iid_ood_mode=iid_ood_mode)
...
#         pointwise_acc = get_pointwise_acc(y, y_pred, rtol=pointwise_acc_rtol, atol=pointwise_acc_atol)
...
#         n_nans = nans.sum()
#         X[nans], y[nans] = get_data(gt_equation, num_variables, supp, n_nans, iid_ood_mode)
#     if nans.any():
#         raise ValueError("Could not sample valid points for equation " f"{gt_equation} supp={supp}")
#     return X, y
...
#             x = np.random.uniform(l - support_length, h + support_length, number_of_points)
#         else:
...
#         raise ValueError("Could not sample valid points for equation " f"{eq.expr} supp={eq.support}")
