...
from ..dataset.sympy_utils import add_additive_constants, add_multiplicative_constants
from . import data
...
def bfgs(pred_str, X, y, cfg):
    lambdify_modules = [{"coth": coth}, "numpy"]

    # Check where dimensions not use, and replace them with 1 to avoid numerical issues with BFGS
    # (i.e. absent variables placed in the denominator)
    y = y.squeeze()
    X = X.clone()
    ...
    c = 0  # pylint: disable=unused-variable  # noqa: F841
    expr = candidate
    ...
    diffs = []
    for i in tqdm(range(X.shape[1])):  # Most time intensive part by far !
        curr_expr = expr
        ...
        diffs.append(diff)
    # breakpoint()
    # diff = [sp.sympify(example).replace(y,xx[1,i]).replace(x,xx[0,i]).replace(z,xx[2,i])-input_batch[i,-1]
    # for i in range(input_batch.shape[0])]
    if cfg.bfgs.normalization_o:
        raise NotImplementedError
        # diff = [x / max_eq for x in diffs]
        # diff = [sympify(example).replace(y,xx[1,i]).replace(x,xx[0,i]).replace(z,xx[2,i])-input_batch[i,-1]/max_eq
        # for i in range(input_batch.shape[0])]
    loss = 0
    cnt = 0  # pylint: disable=unused-variable  # noqa: F841
    if cfg.bfgs.normalization_type == "NMSE":  # and (mean != 0):
        # Needs a re-write & inspiration from better ways to compute this inner loop optomization step,
        # at current far far too slow on multiple accounts
        mean_y = np.mean(y.cpu().numpy())
        # std_y = np.mean(y.cpu().numpy())
        if abs(mean_y) < 1e-06:
    ...
    for i in range(cfg.bfgs.n_restarts):
        ...
        # bfgs optimization - Not working as expected, infinite loop needs solving
        fun_timed = TimedFun(fun=sp.lambdify(s, loss, modules=lambdify_modules), stop_after=cfg.bfgs.stop_time)
        if len(x0):
        ...
        y_found = sp.lambdify(",".join(cfg.total_variables), final, modules=lambdify_modules)(**values)
